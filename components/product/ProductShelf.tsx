...
function ProductShelf({
  ...
  return (
    <Container
      id={id}
      class="grid grid-cols-[48px_1fr_48px] grid-rows-[48px_1fr_48px_1fr] py-10 px-4 sm:px-5 outline-interactive"
    >
      <Console content={products} />
      <h2 class="text-center row-start-1 col-span-full">
        <Text variant="heading-2" class="text([2.8rem] primary-red) font-bold">
          {title}
        </Text>
      </h2>

      <Slider class="gap-1.5 col-span-full row-start-2 row-end-5" // snap="snap-center sm:snap-start block first:ml-6 sm:first:ml-0 last:mr-6 sm:last:mr-0"
      >
        {products?.map((product) => (
          <div class="outline-black w-full !mr-0">
